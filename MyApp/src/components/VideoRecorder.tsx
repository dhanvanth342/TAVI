...
const VideoRecorder: React.FC<VideoRecorderProps> = ({ visible, onClose, onVideoRecorded }) => {
  ...
  }, [visible, isCameraReady, hasPermissions]);
    ...
    try {
      camera.current!.startRecording({
        ...
        onRecordingFinished: async (video) => {
          ...
          // --- FIX: Normalize Android file URI and guard against missing path ---
          let videoPath = video?.path || '';
          if (!videoPath) {
            console.error('❌ No video file path returned from VisionCamera.');
            onClose();
            return;
          }
          if (Platform.OS === 'android' && !videoPath.startsWith('file://')) {
            videoPath = `file://${videoPath}`;
          }
      
          console.log('Sending video path to handler (handleVideoRecorded):', videoPath);
          onVideoRecorded(videoPath);
          onClose();
        },
        onRecordingError: (error) => {
